...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        # Compute Q-value as dot product of features and weights
        qValue = 0.0
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        currentQValue = self.getQValue(state, action)
        nextValue = self.computeValueFromQValues(nextState)
        difference = reward + self.discount * nextValue - currentQValue
        
        # Update weights: w_i <- w_i + alpha * difference * f_i(s,a)
        for feature, value in features.items():
